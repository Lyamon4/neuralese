...
def node(type_name: str, branch_head: bool = False, pass_through: bool = False):
	def deco(fn: NodeFn):
		NODE_REGISTRY[type_name] = fn
		fn.__node_type__ = type_name
		fn.__branch_head__ = branch_head
		fn.__pass_through__ = pass_through
		return fn
	return deco
...
def gen_context(**kwargs):
	ctex = Context(**kwargs)
	ctex.nested = Context(**kwargs)
	ctex.nested.extra["parent"] = ctex
	return ctex
...
class ExecutionResult:
	...
	branch_heads: Dict[str, Dict[str, Any]]
	endings: Dict[str, Dict[str, Any]]
	trace: Dict[Tuple[str, str], Dict[str, Any]]
	inbox_by_page: Dict[str, Dict[str, Dict[str, Any]]]

def pass_emits(emits, node_defs) -> int:
	res = 0
	for port in emits:
		#res[port] = {}
		for target_node in emits[port]:
			if not target_node in node_defs: continue
			if NODE_REGISTRY[node_defs[target_node][1]["type"]].__pass_through__:
				pass
			else:
				res += 1
	return res
...
def execute_graph(pack: Dict[str, Any],
	...
	for idx, page_k in enumerate(page_keys):
		...
		for nid in ready_now:
			...
			if type_name not in NODE_REGISTRY:
				raise KeyError(f"unknown node type {type_name} (node {nid} type {type_name})")
			...
			is_terminal = pass_emits(emit, node_defs) == 0
			is_flagged = getattr(fn, "__branch_head__", False) or bool(props.get("branch_head"))

			if (is_terminal or is_flagged) and not fn.__pass_through__:
				branch_heads[nid] = outputs.copy() if outputs else inputs.copy()
	...
	#print(branch_heads)
	#branch_heads.update(last_inbox)
