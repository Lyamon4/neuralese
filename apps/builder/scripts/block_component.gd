#NEVER rewrite this file or EVERYTHING will fall apart.
@tool
extends ColorRect
...
@export_group("Reveal")
@export var reveal_extents_targets: Array[NodePath] = []
...
func contain(child: BlockComponent):
	...
	if add_to_size:
		expanded_size += _contained_child_height(child) + arrangement_padding.y
...
func _contained_child_height(child: BlockComponent) -> float:
	if not is_instance_valid(child):
		return 0.0
	var height: float = max(child.base_size.y, max(child.size.y, child.custom_minimum_size.y))
	if child._wrapped_in != child and is_instance_valid(child._wrapped_in):
		height = max(height, max(child._wrapped_in.size.y, child._wrapped_in.custom_minimum_size.y))
	return height

func dynamic_child_exit(child: BlockComponent):
	if child in _contained:
		_unclamped_expanded_size -= floor(_contained_child_height(child) + arrangement_padding.y)
...
func arrange():
	...
	for node:BlockComponent in _contained:
		var size_y = _contained_child_height(node)
...
var _applied_reveal_extents: Vector4 = Vector4.ZERO
var _has_applied_reveal_extents: bool = false
...
func _child_needs_reveal_extents(child: BlockComponent, extents: Vector4) -> bool:
	return not child._has_applied_reveal_extents or child._applied_reveal_extents != extents


func _apply_child_reveal_extents(child: BlockComponent, extents: Vector4) -> void:
	child.set_instance_shader_parameter("extents", extents)
	child.label.set_instance_shader_parameter("extents", extents)
	for target_path in child.reveal_extents_targets:
		var target = child.get_node_or_null(target_path)
		if target is CanvasItem:
			target.set_instance_shader_parameter("extents", extents)
	child._applied_reveal_extents = extents
	child._has_applied_reveal_extents = true
...
func update_children_reveal() -> void:
	...
	#(tb)
	var reveal_extents = extents if has_shrink else Vector4.ZERO
	...
	for i in n:
		...
		var child_height = _contained_child_height(c)
		var new_visible = has_shrink or (pos + child_height < max_y)
		var new_free = size_diff_ok and (pos - bar_value < max_y) and (pos - bar_value + child_height > base_y)
		var needs_extents = changed_extents or _child_needs_reveal_extents(c, reveal_extents)

		if c.visible != new_visible or c.freedom != new_free or needs_extents:
			...
			if not new_free:
				...
				a += 1
			if needs_extents:
				_apply_child_reveal_extents(c, reveal_extents)
